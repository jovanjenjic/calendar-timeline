...
const checkAreSubcomponentsExist = () => {
  ...
  cy.get(dayNumber).each((day) => {
    if (day.attr('data-day-type') === 'current') {
      cy.wrap(day).should('have.css', 'background-color', 'rgb(10, 14, 20)');
      ...
      cy.wrap(day).should('have.css', 'color', 'rgb(10, 14, 20)');
    }
  });
};
...
const navigateToNextThenOnToday = () => {
  ...
  cy.get(navigationTimeDateText).should(
    'have.text',
    format(new Date(CURRENT_TIME_DATE), TimeDateFormat.MONTH_YEAR),
  );
};
...
describe('Calendar month view', () => {
  beforeEach(() => {
    cy.mount(
      <CalendarComponent currentView={CurrentView.MONTH} showNavigation />,
    );
  });
